...
현재 버전은 **0.2.13a**입니다. Create 기술 시대 다섯이 모두 들어간 알파 릴리즈이며, 변경 내역은 [CHANGELOG.md](CHANGELOG.md)에 있습니다.
...
| | 튜토리얼 | 262 | 첫 원목부터 엔더 드래곤과 에필로그까지 |
...
- **Toni's Immersive Lanterns** — 랜턴을 허리에 매달아 다닐 수 있게 합니다. Accessories 벨트 칸에 넣으면 걸음에 맞춰 흔들리며 주변을 밝힙니다.
...
- **No Chat Reports** — 채팅 서명을 끄고 서버 접속 시 뜨는 검증 경고를 없앱니다. 서명이 없는 상태를 의도된 것으로 표시합니다.
...
- **TxniLib** — Toni's Immersive Lanterns가 사용하는 기반 라이브러리입니다.
...
```powershell
packwiz curseforge export -o "MoonScenty-Presents-0.2.13a.zip"
```
