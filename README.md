...
| **Atomic Age (원자력 시대)** | Create: Atomic | 공허 강철 티어(256 RPM)를 여기서 엽니다. 최종 생산 체계를 구축하고 강력하거나 특별한 보상형 아이템을 대량 생산해 사용할 수 있습니다. |
...
**원자력 시대의 동력원은 아직 정하지 않았습니다.** 공허 강철 티어가 256 RPM입니다.
...
방수 구리 케이싱과 강철 케이싱, 유령 들린 황동 판은 이 모드팩이 추가한 것입니다. 구리 판은 압착기를, 유령 들린 황동 판은 인케이스드 팬을, 강철 판은 황동 시대의 가공 사슬 전체를 요구하므로 케이싱 하나가 그 시대의 관문이 됩니다.
...
챕터는 여섯 개의 그룹으로 나눠 배치합니다. 현재 18개 챕터에 퀘스트 776개가 들어 있습니다.
...
| [CHANGELOG.md](CHANGELOG.md) | 릴리즈별 변경 기록. 첫 릴리즈 전까지는 비워 둡니다 |
...
기술 축에서 남은 일은 **산업 시대의 퀴스트 챕터와 원자력 시대 전체**입니다. 산업 시대는 설계와 레시피가 끝났고 챕터만 비어 있습니다. 챌린지 미션 챕터도 비어 있습니다.
...
`CHANGELOG.md`는 **첫 릴리즈부터 씁니다.** 그전까지는 변경 내역을 커밋 메시지로만 남깁니다.
...
- FTB Quests의 여섯 그룹 18개 챕터가 오류 없이 열리고 퀘스트 776개의 배치, 목표 판정과 의존 관계가 정상적으로 작동하는지
