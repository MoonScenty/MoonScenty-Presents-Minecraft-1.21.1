...
- More Quest Types로 블록 파괴·설치, 길들이기, 낚시와 거래 같은 행동 목표를 판정합니다.
- 게임이 안정적으로 판정할 수 없는 설명형 목표는 플레이어가 내용을 확인한 뒤 완료하는 체크 과제로 구성합니다.
- Deep Dark와 위더는 에필로그 완료 후에만 보이는 독립 선택 분기이며, 워든 처치는 요구하지 않습니다.
...
- 현재 단계에서는 흐름과 목표 판정 검증에 집중하며 퀘스트 보상을 구성하지 않습니다.
...
위더를 지하 기반 시설이나 Create 공장 가까이에서 소환하지 않도록 반복해서 경고합니다.
...
- 드래곤 처치까지의 기본 및 선택 진행: 212개
- 엔드 에필로그: 13개
- Deep Dark 선택 분기: 20개
- 위더 선택 분기: 18개
- 전체: 1개 챕터, 263개 퀘스트, 269개 판정 과제

초기 구현 이후에는 게임 내 배치, 목표 판정 시점, 멀티플레이 팀 공유와 설명의 정확성을 실제 플레이로 검증합니다.
