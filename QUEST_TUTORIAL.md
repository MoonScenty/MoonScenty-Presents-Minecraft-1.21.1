...
- 각 설계 구역의 마지막 대표 퀘스트에는 해당 장의 루트 체스트를 한 번 지급합니다.
...
- 장별 루트 체스트는 이미 배운 플레이를 돕는 소모품과 보조 재료만 제공하며 다음 기술 단계를 건너뛰게 하지 않습니다.
...
- 부제에는 실제 목표를 한 줄로 요약하고, 설명에는 조작법과 함께 이 목표가 필요한 이유나 이후 쓰임을 1~2문장으로 작성합니다.
- 같은 내용을 반복하거나 지나치게 긴 배경 설명을 넣지 않습니다.
...
각 대표 퀘스트에는 이름이 지정된 전용 리워드 테이블을 연결합니다. 완료 후 보상을 수령하면 테마에 맞는 다섯 후보 중 두 묶음이 직접 지급됩니다. 같은 테이블을 루트 체스트로도 정의하지만 일반 몬스터·동물·보스의 무작위 드롭에는 포함하지 않습니다.

| 장 | 지급 퀘스트 | 보상 방향 |
| --- | --- | --- |
| 0장 | 퀘스트는 거들 뿐 | 식량, 횃불과 기본 재료 |
| 1장 | 돌을 돌려드립니다 | 돌, 연료와 목재 |
| 2장 | 오늘 밤 주인공은 나야 나 | 식량, 조명과 정착 재료 |
| 3장 | 빵이 없으면 밀을 먹...지는 맙시다 | 농사 재료와 식량 |
| 4장 | 철통 방어 | 철기 시대의 소모품과 보조 재료 |
| 5장 | 검고 단단한 그것 | 지하 탐사와 마법 부여 재료 |
| 6장 | 눈을 마주치지 말아요 | 화살과 몬스터 전리품 |
| 7장 | 고쳐 쓰는 것이 미덕 | 청금석, 경험치와 책 재료 |
| 8장 | 마을을 지켜라 | 에메랄드와 주민 거래 물자 |
| 9장 | 바다의 주인은 누구인가 | 지도, 이동과 해양 탐사 물자 |
| 10장 | 톱니바퀴를 돌릴 시간 | 레드스톤 회로 부품 |
| 11장 | 곱게 갈아드립니다 | 네더 생존과 양조 재료 |
| 12장 | 원샷 원회복 | 물약 제작 보조 재료 |
| 13장 | 열두 눈이 지켜보고 있다 | 근거지와 엔드 원정 물자 |
| 14장 | 엔드에서 숙면 금지 | 드래곤전 소모품 |
| 15장 | 집으로 가는 길 | 드래곤 토벌 이후 탐사 물자 |
| 16장 | 끝난 줄 알았지? | 겉날개와 외곽 엔드 활용 물자 |
| 17A장 | 들어갈 때보다 조용히 나왔다 | Deep Dark 잠입과 고대 도시 전리품 |
| 17B장 | 보스도 잡고 별도 따고 | 신호기 확장과 전투 회복 물자 |
