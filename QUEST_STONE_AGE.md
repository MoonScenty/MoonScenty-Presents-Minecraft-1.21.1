...
아연 주괴를 굽는 입력이 셋이고, 각각 화로와 용광로 판본이 있어 레시피는 여섯 개입니다. 전부 Create가 기본으로 제공하며 막지 않습니다.

| 입력 | 얻는 법 | 이 시대에 |
| --- | --- | --- |
| `c:raw_materials/zinc` | 아연 광석을 곡괭이로 캠 | 사용 가능 |
| `c:ores/zinc` | 실크터치로 광석 블록째 캠 | 사용 가능 |
| `create:crushed_raw_zinc` | 분쇄바퀴로 원광을 갈아야 나옴 | **불가** — 분쇄바퀴가 황동 케이싱을 요구 |

그래서 석기 시대의 선택지는 앞의 둘이고 수율은 같습니다. 분쇄바퀴가 열리는 시점부터 세 번째 경로에서 수율이 올라가는데, 이는 README의 *"다음 시대에 진입하면 이전 시대의 자원은 더 쉽게 생산할 수 있어야 한다"* 는 원칙에 맞습니다.

주괴와 별개로 합금용 아연은 다른 길로 갑니다. `createmetallurgy:zinc_dust`는 Metallurgy에 아이템만 있고 만드는 레시피가 없던 것을 이 모드팩이 밀링으로 열었습니다. 이 가루는 주괴가 되지 않고 용해로에서 용융 아연이 되는 합금 전용입니다.

```
원광 아연 ─┬─ 화로 / 용광로 ──────→ 아연 주괴      승급 철 블록용
           └─ 맷돌 → 아연 가루 → 용해로 → 용융 아연   합금용
```
...
별도 게이팅이 필요 없습니다. 재료는 석기 시대에도 있지만 **압착기가 안산암 케이싱을 요구**하므로 흑연 덩어리에서 멈춥니다.

```
점토 1 + 석탄 8  →  Graphite              제작대
Graphite         →  Graphite Blank Mold   기계식 압착기   ← 여기가 관문
Blank Mold       →  잉곳 / 너깃 / 판 / 봉 / 기어   석재 절단
```

압착 레시피는 `data/create/recipe/pressing/graphite.json`에 있습니다. Metallurgy가 자기 네임스페이스가 아니라 `create` 쪽에 넣어 두었습니다.

두 등급의 성격이 이렇게 갈립니다.

| | 재료 | 필요 설비 | 소모 |
| --- | --- | --- | --- |
| 내화 모르타르 | 모래 8 + 점토 8 | 없음 | 주조 1회 |
| 흑연 | 점토 1 + 석탄 8 | 압착기 | 재사용 |

석기 시대에는 매번 만들어 쓰다가, 압착기가 생기면 한 번 만들어 계속 쓰는 쪽으로 갈아탑니다.
...
수치 조정은 **모드팩 릴리즈 후 의견을 모아 진행합니다.** 혼자 재는 것보다 여러 사람이 실제로 플레이한 결과가 낫습니다.

지금 잡아 둔 값은 이렇습니다.

| 항목 | 값 |
| --- | --- |
| 안산암 합금 1개 | 안산암 1 + 원광 아연 1 + 모래 8 + 점토 8 |
| 야금 설비 일습 | 승급 철 블록 34개 = 철 17 + 아연 34 |
| 아연 배율 | Create 기본(안산암 1 + 아연 너깃 1) 대비 아연 9배 |

모래와 점토는 거푸집이 일회용이라 생기는 유지비입니다. 손잡이가 셋 있습니다 — 내화 모르타르 제작법의 출력 개수, 빈 거푸집이 먹는 볼 개수, 거푸집을 여러 번 쓰게 하는 것입니다.
